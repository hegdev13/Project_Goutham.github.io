...
The core aspect of the course is to understand the intuition behind the problem-solving approach. Who better to learn about problem-solving than Mother Nature?

Nature's most significant problem is of sustaining life, And to that end, it counters all problems that threaten the quality and extinction of life. Learning the way these problems are structured and how natural elements solve them with techniques that have taken eons to build and are refined by the process of evolution, develops intuition for identifying and optimizing solutions. 

For the human simplicity, we classify the vast number of natural phenomena into 3 patterns of problem solving approaches: iteration, recursion and backtracking. 
...
Once we figure out a way to solve a problem, the next step to be considered is the resources that the solution takes, and how well it performs. 
...
Time efficiency: The time that an algorithm takes to run with respect to the input size metric. It is directly related to Basic Operation and Basic count.
...
We borrow the notations of Omega (Ω), Theta(𝜭) and Big-Oh (O) to describe the best, average and worst cases of performance, better known as **Complexity**,  of an algorithm. These 3 notations satisfactorily describe the efficiency of an algorithm in terms of orders of growth. 
...
- O(1): Constant 
	- An algorithm is said to have constant time complexity if the time taken to execute the algorithm is constant with varying sizes of inputs.
	- An algorithm is said to have constant space complexity if constant extra space is taken up during the execution of the algorithm with respect to the input size.
	  (The following are defined similarly)
- O(log n): Logarithmic 
- O(n): Linear 
